...
## v2.2-alpha: guided tutorial
Added an in-app **Tutorial** (☰ menu → Take the Tutorial). It walks through setting up a first hunt one step at a time — opening the right tab, scrolling to the relevant control, and putting a pulsing highlight ring around exactly what to tap or fill in, with Back/Next/Skip controls in a bar at the bottom of the screen. Covers hunting focus → location → radius → chains → local shops → item types → HUNT! → Start/End → Generate Route, in order. It's manually triggered from the menu for now rather than auto-launching on first visit — easy to add later if that's wanted.
